import requests
from configs.api_parser import *
from helpers.api_logic import *
...
class APIUserPage:
    ...
    def create_user(self):
        self.user = requests.post(create_user, json=creation_of_user())
        return self.user.status_code

    def login(self):
        self.user = requests.get(log_in)
        return self.user.status_code

    def get_user_info(self):
        self.user = requests.get(get_user_info).json()
    ...
    def logout(self):
        self.user = requests.get(log_out)
        return self.user.status_code
...
class APIPetPage:
    ...
    def create_pet(self):
        self.pet = requests.post(create_pet, json=creation_of_pet())
        return self.pet.status_code

    def pet_info(self):
        self.pet = requests.get(find_pet).json()
        return self.pet
    ...
    def update_pet_name(self):
        self.pet = requests.put(update_pet, json=update_pet_name())
        find = requests.get(find_pet).json()
        return find["name"]
